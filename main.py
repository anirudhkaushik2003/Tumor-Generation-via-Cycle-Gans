...
if __name__ == "__main__":
    opt = TrainOptions()
    opt, _ = opt.initialize().parse_known_args()

    BATCH_SIZE = opt.batch_size
    IMAGE_SIZE = opt.image_size
    IMG_CHANNELS = opt.image_channels
    ...
    # optimizers
    learning_rateG = opt.learning_rateG
    learning_rateD = opt.learning_rateD
    ...
    elif opt.criterion == "BCEwithLogits":
    ...
    ### PRINT STATS ###
    print("***********************")
    print(f"Discriminator Architechture: {architechture}")
    ...
    if opt.wandb:
        wandb_init(opt.wandb_project, opt.run_name, learning_rateG, learning_rateD, epochs*2, BATCH_SIZE, patch_shape, architechture, "BHB->BRATS", multiGPU)


    # check if latest file exists
    if os.path.exists(f"/ssd_scratch/cvit/anirudhkaushik/checkpoints/tumor_ganG1_{opt.save_name}_checkpoint_latest.pt"):
        print("***********************")
        print("Loading latest checkpoint...")
        restart_last_checkpoint(modelG_1, optimizerG, multiGPU, "G1_{opt.save_name}")
        restart_last_checkpoint(modelD_1, optimizerD, multiGPU, "D1_{opt.save_name}")
        restart_last_checkpoint(modelG_2, optimizerG, multiGPU, "G2_{opt.save_name}")
        restart_last_checkpoint(modelD_2, optimizerD, multiGPU, "D2_{opt.save_name}")
        print("***********************", end="\n\n\n\n")
    ...
    for epoch in range(epochs*2):
        ...
        for step, (healthy, (tumor, tumor_label)) in enumerate(zip(dataloader, dataloader2)):
            ...
            if architechture == "patchGAN":
                ones = torch.ones((int(healthy.shape[0]), 1, patch_shape, patch_shape)).to(device)
            elif architechture == "UNET":
                ones = tumor_label.to(device) # condition label with tumor segmentation mask
            ...
            loss_G = train_composite_model(modelD_1, modelD_2, fake_tumor, fake_healthy, rec_tumor, rec_healthy, tumor, healthy, ones, id=opt.id)
            # add to lost list
        ...
        create_checkpoint(modelG_1, epoch, multiGPU, f"G1_{opt.save_name}")
        create_checkpoint(modelG_2, epoch, multiGPU, f"G2_{opt.save_name}")
        create_checkpoint(modelD_1, epoch, multiGPU, f"D1_{opt.save_name}")
        create_checkpoint(modelD_2, epoch, multiGPU, f"D2_{opt.save_name}")
